...
exports.agregarProyecto = async(req, res) => {
    if (req.files) {
        ...
        let estudiantes = [];
        console.log("Se está recibiendo en estudiantes ...", req.body.estudiante)
        if (Array.isArray(req.body.estudiante)) {
          estudiantes = req.body.estudiante.map(id => mongoose.Types.ObjectId(id));
        } else if (typeof req.body.estudiante === 'string') {
          // Si es un string, puedes convertirlo directamente a ObjectId
          estudiantes = req.body.estudiante.split(',').map(id => new mongoose.Types.ObjectId(id.trim()));
        }       
        ...
        //const nuevoProyecto = new guardar
        ...
        if (resp.isOk) {
            ...
            try {
                ...
                console.log("nuevoProyectoApi", nuevoProyecto);
...
exports.descargarArchivo = async (req, res) => {
    try {
      ...
      const filePath = path.resolve(__dirname, '../../archivos', proyecto.nombreDocumento); // Construir la ruta absoluta del archivo
  ...
  exports.verArchivo = async (req, res) => {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        res.sendFile(filePath, { headers: { 'Content-Disposition': 'inline' } }); // Para abrir en el navegador
      } else {
        console.log('Archivo no encontrado en el servidor');
        res.status(404).json({ error: 'Archivo no encontrado en el servidor' });
      }
